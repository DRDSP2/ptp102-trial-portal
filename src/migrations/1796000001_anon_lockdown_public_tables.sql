-- 1796000001: Anonymous-lockdown for the 14 public tables that still had RLS
-- disabled (and were therefore readable/writable by the anon key under
-- Supabase defaults).
--
-- Investigation (static, across src/migrations, supabase/migrations, and
-- scripts/migrations_combined.sql) found NO anon-granting policies; the only
-- exposure was RLS being OFF. These 14 tables were created by:
--   - 1738500000_create_compliance_framework.sql  (13 compliance/study tables)
--   - 1730759100_create_admin_users_table.sql     (admin_users)
-- Every other public table already has RLS enabled with NDA (deal-portal) or
-- verified-vet/admin (clinical) gating from earlier migrations.
--
-- Fix per approved scope:
--   - 13 compliance/study tables: enable RLS; SELECT/INSERT/UPDATE allowed for
--     is_admin() OR is_verified_vet(); DELETE admin-only.
--   - admin_users: enable RLS; all commands admin-only (credential table).
--   - is_verified_vet() (1796000000) and is_admin() (042/048) are reused.
...
CREATE POLICY "study_settings_select" ON public.study_settings FOR SELECT TO authenticated
  USING (is_admin() OR is_verified_vet());
...
CREATE POLICY "study_settings_insert" ON public.study_settings FOR INSERT TO authenticated
  WITH CHECK (is_admin() OR is_verified_vet());
...
CREATE POLICY "study_settings_update" ON public.study_settings FOR UPDATE TO authenticated
  USING (is_admin() OR is_verified_vet()) WITH CHECK (is_admin() OR is_verified_vet());
...
-- ============================================================================
-- 052: tighten deal-portal policies that defaulted to TO PUBLIC
--   deal_team_members / deal_pipeline_entries had `USING (true)` with no TO
--   clause, so Postgres applied them to PUBLIC (incl. anon) -> anonymous read.
