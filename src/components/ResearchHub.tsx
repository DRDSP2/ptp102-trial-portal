      ...
      content: (
        <div className="space-y-4">
          ...
          <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-sm font-semibold text-blue-800 mb-2">Key Research Finding</p>
            <blockquote className="text-sm text-muted-foreground italic border-l-2 border-info pl-3">
          ...
          <Dialog key={section.id}>
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-[#6b7f3a]/10 rounded-lg text-primary group-hover:bg-[#6b7f3a] group-hover:text-white transition-colors">
                          {section.icon}
                        </div>
                        <div>
                          <CardTitle className="text-base sm:text-lg">{section.title}</CardTitle>
                          {section.badge && (
                            <Badge variant="outline" className="mt-1 text-[10px] border-[#6b7f3a]/30 text-primary">
                              {section.badge}
                            </Badge>
                          )}
                        </div>
            ...
            <DialogContent className="max-w-2xl max-h-[85vh] overflow-hidden p-0">
              <DialogHeader className="px-6 pt-6 pb-3 border-b">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-[#6b7f3a]/10 rounded-lg text-primary">
                    {section.icon}
                  </div>
      ...
      <Card className="border-dashed border-border bg-muted/40">
        <CardContent className="p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <p className="font-semibold text-sm text-foreground">Need additional research materials?</p>
            ...
            <Button
              variant="outline"
              size="sm"
              className="border-[#6b7f3a]/30 text-primary hover:bg-[#6b7f3a] hover:text-white"
...
function AlertBanner({ type, title, message }: { type: 'info' | 'warning'; title: string; message: string }) {
  const styles = {
    info: 'bg-info/10 border-info/40 text-blue-800',
    warning: 'bg-warning/10 border-warning/40 text-amber-800',
  };
  return (
